...
function Singlepage() {
  ...
  const getsingleBook = async () => {
    console.log('here object length',Object.keys(book).length);
    if(!id)
    return;
    try {
      const response = await axiosInstance.get(`/book/singlebook/${id}`);
  ...
  useEffect(() => {
    console.log("Fetching book for name:", id);
    getsingleBook();
  }, []);

  let content;
  if (!Object.keys(book).length) {
    content = <h1 className="text-xl items-center">No Book is available</h1>;
  } else {
    content = (
